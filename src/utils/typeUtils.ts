...
export function createStringEnum<E extends z.ZodEnum<any>>(
  ...
  return Object.assign(record, {
    values: e.options,
    getName: (value: E["def"]["entries"][keyof E["def"]["entries"]]) =>
      e.def.entries[value],
...
export type Replace<T, TKey extends keyof T, TNew> = Omit<T, TKey> & {
  [K in TKey]: TNew;
};
...
type _AllUnionFields<
  T,
  K extends PropertyKey = T extends unknown ? keyof T : never,
> = T extends unknown ? T & Partial<Record<Exclude<K, keyof T>, never>> : never;

// Shallow union
export type AllUnionFields<T> = {
  [K in keyof _AllUnionFields<T>]: _AllUnionFields<T>[K];
};
...
export type DeepAllUnionFields<T> =
  IsPlainObject<T> extends true
    ? {
        [K in keyof _AllUnionFields<T>]: DeepAllUnionFields<
          _AllUnionFields<T>[K]
        >;
      }
    : T;
...
type PropOfAll<T, K extends PropertyKey> = T extends unknown
  ? K extends keyof T
    ? T[K]
    : never
  : never;
...
export type DeepSharedUnionFields<T> =
  IsPlainObject<T> extends true
    ? {
        [K in keyof UnionToIntersection<T>]: DeepSharedUnionFields<
          PropOfAll<T, K>
        >;
      }
    : T;
