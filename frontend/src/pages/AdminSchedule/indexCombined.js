...
function Schedule() {
    ...
    const fetchShifts = useCallback(async () => {
        ...
        const filteredShifts = uniqueShifts.filter((shift) => {
            ...
            if (isAdmin) {
              return shift.coverage_status === filter; // Filter logic based on shift.coverage_status
          } else {
    ...
    const generateButtonsForDetailsPanel = (shift) => {
        ...
        const primaryButton = buttonConfig[shift.shift_type] || buttonConfig[shift.coverage_status] || buttonConfig[SHIFT_TYPES.DEFAULT]; // shift.coverage_status for admin shifts
