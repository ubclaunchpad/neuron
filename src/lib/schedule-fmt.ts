...
export type ListSeparators = {
    middle: string; // between items except the last two
...
export function joinWithSeparators(
    ...
    const { middle, final } = separators;

    if (items.length === 0) return "";
    if (items.length === 1) return items[0]!;
    if (items.length === 2) return `${items[0]}${final}${items[1]}`;

    const allButLast = items.slice(0, -1).join(middle);
    const last = items[items.length - 1];
    return `${allButLast}${final}${last}`;
}
...
export type Recurrence = WeeklyRecurrence | MonthlyRecurrence | SingleRecurrence;
...
function formatWeeklyRecurrence(
    ...
    if (style === "short") {
        ...
        return `Every ${interval} weeks on ${weekday}`; // "Every 2 weeks on Mon"
    }
