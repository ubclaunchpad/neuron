  ...
  return (
    <div className="form-container">
      ...
      <Form onSubmit={handleSubmit(onSubmit)} validationBehavior="aria" className="form-content">
        { isSubmitSuccessful ? (
          <p>
            We have sent the password reset instructions to <strong>{getValues('email')}</strong><br/>
            Did not receive the email? Check your spam folder or click below to resend instructions. 
          </p>
        ) : (
          <TextInput
              type="email"
        ...
        <Button type="submit" isDisabled={isActive}>
          {isActive ? <strong>{formatted}</strong> : isSubmitSuccessful ? 'Resend Instructions' : 'Send Instructions'}
        </Button>
