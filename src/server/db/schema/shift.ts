...
export const shift = pgTable("shift", {
    ...
    courseId: uuid("class_id").notNull().references(() => course.id, { onDelete: "cascade" }),
    ...
    index("idx_shift_date").on(table.date, table.courseId).where(not(table.canceled)),
...
export const attendanceStatus = pgEnum("attendance_status", AttendanceStatus.values);

export const shiftAttendance = pgTable("shift_attendance", {
    shiftId: uuid("shift_id").notNull().references(() => shift.id, { onDelete: "cascade" }),
    userId: uuid("user_id").notNull().references(() => volunteer.userId, { onDelete: "restrict" }),
    status: attendanceStatus("status").notNull(),
    checkedInAt: timestamp("checked_in_at", { withTimezone: true }),
    minutesWorked: smallint("minutes_worked"),
}, (table) => [
    primaryKey({ name: "pk_shift_attendance", columns: [table.shiftId, table.userId] }),
