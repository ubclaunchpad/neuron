...
export class StringEnum<T extends string> {
    ...
    static createFromType<
        ...
        TMap = Record<TEnum, string>
    >(keys: TMap): StringEnum<TEnum> & { [K in keyof TMap]: K } {
    ...
    getSelectOptionsWithInterpolation(fn: (key?: string) => string | undefined, blankOption = false): SelectEntity[] {
    ...
    getSelectOptions(blankOption: boolean = false): SelectEntity[] {
        return this.getSelectOptionsWithInterpolation((k) => k, blankOption);
    }

    getSelectOptionsWithSearch(searchStr?: string, blankOption: boolean = false): SelectEntity[] {
        const valueToFilterOn = (searchStr || '').toLowerCase();
        return this.getSelectOptions(blankOption).filter(x => x.label.toString().toLowerCase().indexOf(valueToFilterOn) > -1);
    }
