...
type PrimitiveArrayPathInternal<T, TraversedTypes = T> =
  T extends ReadonlyArray<infer V>
    ? IsTuple<T> extends true
      ? {
          [K in TupleKeys<T>]-?: PrimitiveArrayPathImpl<
            K & string,
            T[K],
            TraversedTypes
          >;
    ...
    : {
        [K in keyof T]-?: PrimitiveArrayPathImpl<
          K & string,
          T[K],
          TraversedTypes
        >;
 ...
 */
export type PrimitiveArrayPath<T> = T extends any
  ? PrimitiveArrayPathInternal<T>
  : never;
...
export type PathValue<
  T,
  P extends Path<T> | ArrayPath<T> | PrimitiveArrayPath<T>,
> = PathValueImpl<T, P>;
...
export type PrimitiveFieldArrayPathByValue<
  TFieldValues extends FieldValues,
  TValue,
> = {
  [Key in PrimitiveFieldArrayPath<TFieldValues>]: PrimitiveFieldArrayPathValue<
    TFieldValues,
    Key
  > extends TValue
    ? Key
...
>(params: {
  ...
  const { name } = params;
  const form = useFormContext<TFieldValues, any, TTransformedValues>();
